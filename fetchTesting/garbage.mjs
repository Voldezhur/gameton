...
function sortGarbageForCompartment(garbageData, compartmentRows, compartmentCols, compartmentStatus) {
  ...
  const planetGarbage = garbageData.find(item => 'planetGarbage' in item).planetGarbage;

  const fillPercentageEmpty = 0.3;
  const fillPercentageNonEmpty = 0.05;

  const flattenCompartment = compartment.flat();
  let currentFillCount = flattenCompartment.filter(coord => coord !== null).length;
  
  const fillPercentage = compartmentStatus === 'empty' ? fillPercentageEmpty : fillPercentageNonEmpty;
  const targetFillCount = Math.floor(compartmentRows * compartmentCols * fillPercentage);
...
const garbageData = [{"planets":["Tromp-232","Borer-147"]},
{"planetGarbage":{"2FwG63":[[0,0],[1,0],[2,0],[3,0],[1,1],[1,2],[1,3],[2,3]],"2Fwbiq":[[0,0],[1,0],[2,0]],"2Fwvu8":[[0,2],[0,1],[0,0],[1,2],[1,0],[2,2],[3,2],[3,1],[3,0]],"2FxGXv":[[0,0],[0,1],[1,1],[2,1],[3,1],[1,2],[1,3],[2,3],[3,3]]}},
{"shipGarbage":null,"fuelDiff":79,"planetDiffs":[{"from":"Johnston-120","to":"Tromp-232","fuel":74},{"from":"Tromp-232","to":"Borer-147","fuel":25}]}
];
